...
export async function runEnv(cmd: EnvCommand, opts: { schema?: string } = {}) {
  ...
  if (cmd === "generate") {
    const keys = [...new Set([...(schema.required || []), ...(schema.optional || [])])];
  ...
  if (cmd === "diff") {
    const keys = new Set<string>([...Object.keys(example), ...Object.keys(env)]);
    ...
    if (onlyInExample.length === 0 && onlyInEnv.length === 0) {
      log.ok(".env and .env.example are in sync");
    } else {
      if (onlyInExample.length) log.warn(`Missing in .env: ${onlyInExample.join(", ")}`);
      if (onlyInEnv.length) log.warn(`Extra in .env: ${onlyInEnv.join(", ")}`);
      process.exitCode = 1;
    }
    return;
  }
  ...
  if (cmd === "pull:railway" || cmd === "push:railway") {
    log.warn("Railway integration is a stub in this version. Configure API token and implement in future release.");
    return;
  }
}
