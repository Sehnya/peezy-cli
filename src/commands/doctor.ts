...
function checkCmd(cmd: string): { ok: boolean; version?: string; error?: string } {
  try {
...
function findProjectRoot(cwd: string) {
  ...
  while (dir !== path.parse(dir).root) {
    if (fs.existsSync(path.join(dir, "package.json")) || fs.existsSync(path.join(dir, "pyproject.toml")) || fs.existsSync(path.join(dir, "requirements.txt"))) {
      return dir;
    }
...
export async function doctor(opts: DoctorOptions = {}): Promise<number> {
  ...
  if (fs.existsSync(envExample)) {
    const example = fs.readFileSync(envExample, "utf8").split(/\r?\n/).filter(Boolean);
  ...
  if (opts.fixLint) {
    try {
      execSync("npm run lint --silent -- --fix", { cwd: root, stdio: "inherit" });
    } catch {
